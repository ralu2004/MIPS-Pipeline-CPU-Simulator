...
\subsubsection*{I-Type Instruction}
...
\subsubsection*{J-Type Instruction}
...
\subsubsection*{Instruction Factory}

The \textbf{InstructionFactory} utility class provides centralized parsing of 32-bit binary instruction words into corresponding instruction objects using a factory pattern.
...
The factory examines the 6 most significant bits (opcode) to determine instruction format:
\begin{itemize}
    \item Opcode \texttt{0x00} indicates R-type instruction
    \item Opcodes \texttt{0x02} or \texttt{0x03} indicate J-type instructions
    \item All other opcodes indicate I-type instructions
\end{itemize}
...
To enable web-based interaction with the simulator, a lightweight HTTP API server was implemented using Java's built-in \texttt{HttpServer}. This API provides a RESTful interface for controlling the simulation and querying CPU state.

\subsubsection*{API Architecture}

The \textbf{ApiServer} class manages HTTP request handling and coordinates between the simulation engine and external clients. It maintains references to the \texttt{CPUState}, \texttt{PipelineController}, and \texttt{Clock} instances, ensuring a single source of truth for the simulation state.
...
This enables frontend applications hosted separately from the backend to make API requests without browser security restrictions.
...
This design ensures that the frontend receives meaningful error messages and can gracefully handle exceptional conditions during simulation.
\end{comment}
