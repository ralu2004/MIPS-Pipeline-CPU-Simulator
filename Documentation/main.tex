...
In traditional computer architecture courses, students often struggle to understand the functioning of the CPU pipeline and the types of hazards that arise during instruction execution. Existing tools, such as MARS \cite{mars} and SPIM \cite{spim} simulators, allow users to run MIPS assembly programs but do not provide an intuitive visualization of how instructions propagate through the pipeline.
...
\section{Objectives and Proposed Solution}\label{sec:objectives}
...
\begin{itemize}
    ...
    \item The possibility to simulate 19 MIPS instructions (R, I, and J types).
...
The simulator implements 19 core MIPS instructions covering arithmetic, memory, and control flow operations:
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{fig/use_case_diagram.png}
...
The system's class structure is illustrated in Figure~\ref{fig:class_diagram}. The simulator is centered around the \textbf{PipelineController}, which manages clock cycles, pipeline stage coordination, and hazard handling. Each pipeline stage—\textbf{FetchStage}, \textbf{DecodeStage}, \textbf{ExecuteStage}, \textbf{MemoryStage}, and \textbf{WriteBackStage}—is represented by a dedicated class responsible for its specific functionality.

\begin{landscape}
\thispagestyle{empty}
\begin{figure}[H]
    \centering
    \includegraphics[width=1.315\textwidth]{fig/class_diagram.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{fig/data_flow.png}
...
\begin{itemize}
    ...
    \item \textbf{model.cpu:} Implements CPU state components, including the register file and program counter.
    \item \textbf{model.control:} Houses the control unit, hazard detection unit, forwarding unit, and stall unit.
    \item \textbf{model.memory:} Implements the instruction and data memories.
...
\begin{itemize}
    \item \textbf{setInstruction(int, Instruction)} stores an instruction at a given address.
    \item \textbf{fetch(int)} retrieves the instruction from a given address.
\end{itemize}
...
\begin{itemize}
    \item \textbf{loadWord(int)} returns 0 for invalid addresses.
    \item \textbf{storeWord(int, int)} does nothing for invalid addresses.
\end{itemize}
...
The \textbf{HazardDetectionUnit} implements the hazard detection mechanism described in Section~\ref{ch:design}. It monitors the \textbf{PipelineRegisters} to detect data, control, and structural hazards through dedicated methods: \textbf{checkDataHazards(regs)}, \textbf{checkControlHazards(regs)}, \textbf{checkStructuralHazards(regs)}. 
...
The frontend of the Simulator is implemented as a React web application which provides an interactive visualization interface for the simulation as depicted in Figure \ref{fig:ui}. Built with modern React patterns and Tailwind CSS, it offers real-time visualization of pipeline stages, register states, and memory contents.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig/ui.png}
...
\begin{itemize}
    \item \textbf{Trace Mode:} a vertical stack of all five stages with instruction information, such as including hex codes, program counters, and destination registers; control signals display, and hazards display can be enabled/disabled by the user;
    \item \textbf{Learning Mode:} horizontal representation with color-coded stages and integrated data path visualization, showing real-time component activity including pipeline registers components, register file operations, ALU computations, and memory access patterns, as well as learning content;
    \item \textbf{Timeline Diagram:} provides temporal analysis through a cycle-by-cycle tabular display that tracks instruction progression and visually identifies pipeline hazards through color-coded states, including stalls, bubbles, and flushes.
\end{itemize}
...
The 100\% pass rate across diverse test cases demonstrates that each component functions correctly in isolation and integrates properly within the pipeline, confirming that the design specifications are successfully met.

\chapter{Conclusions}\label{ch:conlusions}

This project successfully builds a CPU simulator that models the 5-stage MIPS pipeline architecture with comprehensive visualization capabilities. The simulator addresses a significant gap in computer architecture education by making the invisible internal operation of a pipelined processor visible and interactive. 

\section{Achievement of Objectives}
All objectives established in Section \ref{sec:objectives} were accomplished. The simulator implements 19 MIPS instructions (R-type, I-type, and J-type), simulates all five pipeline stages (IF, ID, EX, MEM, WB), and provides complete hazard detection and resolution mechanisms. An intuitive graphical user interface provides real-time visualization of all core components and learning instruments.

\section{Limitations}
The simulator focuses on educational fundamentals rather than complete realism. It implements 19 core MIPS instructions but omits multiplication, division, and floating-point operations. The memory model uses simplified Harvard architecture without cache hierarchies or latency modeling. Branch prediction employs a basic predict-not-taken strategy rather than sophisticated modern mechanisms. 

\section{Lessons Learned}
Implementing the simulator deepened the developer's understanding of the subtle interactions between pipeline stages and the complexity of maintaining correctness during concurrent instruction execution. 
The comprehensive testing strategy proved essential. Several subtle bugs were discovered only through integration testing with complex instruction sequences that exercised multiple hazard scenarios simultaneously.

\section{Final Remarks}
The MIPS pipeline simulator successfully bridges the gap between theoretical computer architecture instruction and practical understanding. By making pipeline operation visible and interactive, it enables students to develop intuition about instruction-level parallelism, hazard detection, and performance optimization. 
